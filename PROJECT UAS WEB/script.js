...
// REVISI KUAT: Deteksi jika halaman di-refresh, paksa hapus semua data session!
...
function initDashboard() {
  ...
  let bookedSlots = JSON.parse(sessionStorage.getItem("runtimeBookedSlots")) || [];
  let bookingHistory = JSON.parse(sessionStorage.getItem("runtimeActiveBookings")) || [];

  // Sembunyikan slot yang sudah dibooking pada sesi ini
  buttons.forEach((btn) => {
    if (bookedSlots.includes(btn.getAttribute("data-time"))) {
      btn.style.display = "none";
    } else {
      btn.style.display = "block"; // Memastikan tombol muncul kembali saat bersih
    }
  });
  ...
  function updateSlotCount() {
    if (slotCountText) {
      const available = Array.from(buttons).filter((btn) => btn.style.display !== "none").length;
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      buttons.forEach((b) => b.classList.remove("bg-primary-container", "ring-2", "ring-primary"));
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener("click", () => {
      ...
      if (!bookedSlots.includes(selectedTime)) {
        ...
        sessionStorage.setItem("runtimeBookedSlots", JSON.stringify(bookedSlots));
      }
      ...
      sessionStorage.setItem("runtimeActiveBookings", JSON.stringify(bookingHistory));
...
function initHistory() {
  ...
  function renderTables() {
    const allBookings = JSON.parse(sessionStorage.getItem("runtimeActiveBookings")) || [];
    const now = Date.now();
    ...
    completedList.forEach((item) => {
      completedHtml += `
        <tr class="hover:bg-neutral-50/50 transition opacity-75 bg-white/60">
          ...
          <td class="p-4 text-neutral-400 font-mono">0${item.telepon}</td>
          <td class="p-4 text-neutral-400">
